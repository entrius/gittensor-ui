...
const buildMiner = (
  ...
  return {
    ...
    attested,
    tokens,
    // agentic traffic: ~30 prompt tokens per output token
    promptTokens: tokens * 30,
    tokens24h: tokens * 288,
    promptTokens24h: tokens * 30 * 288,
...
export const mockServingStatus = (): ServingStatus => {
  ...
  return {
    ...
    promptTokens: miners.reduce((sum, m) => sum + m.promptTokens, 0),
    totalTokens: miners.reduce((sum, m) => sum + m.tokens + m.promptTokens, 0),
    tokensLast24h: miners.reduce((sum, m) => sum + m.tokens24h, 0),
    promptTokensLast24h: miners.reduce((sum, m) => sum + m.promptTokens24h, 0),
    totalTokensLast24h: miners.reduce(
      (sum, m) => sum + m.tokens24h + m.promptTokens24h,
      0,
    ),
    requestsLast24h: 96 * 287,
    poolShare: 0.0212,
    poolCap: 0.1,
    gpuHourUsd: 0.7,
    usdPerMTokens: 0.694,
