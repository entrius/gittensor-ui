...
const FileItem: React.FC<{
    ...
    const getIcon = () => {
        if (node.type === "tree") {
            return open ? <FolderOpenIcon sx={{ fontSize: 18, color: "#8b949e" }} /> : <FolderIcon sx={{ fontSize: 18, color: "#8b949e" }} />;
        }
        ...
        <>
            <ListItemButton
                ...
                sx={{
                    ...
                    minHeight: 24,
                    height: 24,
                    backgroundColor: isSelected ? "rgba(56, 139, 253, 0.15)" : "transparent",
                    borderLeft: isSelected ? "2px solid #388bfd" : "2px solid transparent",
                    "&:hover": {
                        backgroundColor: isSelected ? "rgba(56, 139, 253, 0.15)" : "rgba(255, 255, 255, 0.04)",
                    },
                    transition: "all 0.1s ease-in-out",
                }}
            >
                <ListItemIcon sx={{ minWidth: 20, mr: 0.5, display: "flex", alignItems: "center", justifyContent: "center" }}>
                    {node.type === "tree" && (
                        <Box component="span" sx={{ display: "flex", alignItems: "center", opacity: 0.7 }}>
    ...
    return (
        <Box sx={{ width: "100%", height: "100%", overflowY: "auto", overflowX: "hidden" }}>
            <List component="nav" dense>
                {files.map((node) => (
                    <FileItem key={node.path} node={node} level={0} onSelect={onSelectFile} selectedFile={selectedFile} />
                ))}
            </List>
        </Box>
    );
