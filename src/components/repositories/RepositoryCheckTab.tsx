...
import FactCheckIcon from "@mui/icons-material/FactCheck"; // For Health Score
...
const RepositoryCheckTab: React.FC<RepositoryCheckTabProps> = ({
    ...
    const [goodFirstIssueCount, setGoodFirstIssueCount] = useState<number | null>(null);
            ...
            try {
                ...
                try {
                    const [openIssuesRes, gfiRes, hwRes] = await Promise.all([
                        axios.get(`https://api.github.com/search/issues?q=repo:${repositoryFullName}+is:issue+is:open&per_page=1`),
                        axios.get(`https://api.github.com/search/issues?q=repo:${repositoryFullName}+is:issue+is:open+label:"good+first+issue"&per_page=1`),
                        axios.get(`https://api.github.com/search/issues?q=repo:${repositoryFullName}+is:issue+is:open+label:"help+wanted"&per_page=1`)
                    ]);
    ...
    const checks: HealthCheck[] = useMemo(() => {
        ...
        const hasFile = (pattern: RegExp) => fileTree.some((path) => pattern.test(path));
        ...
        return [
            ...
            },
            {
                ...
                passed: hasFile(/^(docs\/)?CONTRIBUTING\.(md|txt|rst)$/i) || (communityProfile?.files?.contributing !== null && communityProfile?.files?.contributing !== undefined),
                description: "Guidelines for new contributors.",
            },
            {
                name: "Code of Conduct",
                passed: hasFile(/^(docs\/)?CODE_OF_CONDUCT\.(md|txt|rst)$/i) || (communityProfile?.files?.code_of_conduct !== null && communityProfile?.files?.code_of_conduct !== undefined),
                description: "Standards for community behavior.",
            },
            {
                name: "Pull Request Template",
                passed: hasFile(/^(\.github\/)?PULL_REQUEST_TEMPLATE\.(md|txt)$/i) || hasFile(/^(\.github\/)?PULL_REQUEST_TEMPLATE\//i),
            ...
            <Grid container spacing={2}>
                ...
                <Grid item xs={12} md={4}>
                    <Box sx={{ display: "flex", flexDirection: "column", gap: 2, height: "100%" }}>
                        ...
                        >
                            ...
                            >
                                <CircularProgress
                                    ...
                                    thickness={4}
                                    sx={{
                                        color: score > 80 ? "#238636" : score > 50 ? "#e3b341" : "#da3633",
                                    }}
                                />
                                <Box
                                    sx={{
                            ...
                            <Typography variant="h6" sx={{ color: "#fff", mb: 0.5, fontSize: "16px" }}>
                                Health Score
                            </Typography>
                            <Typography variant="body2" sx={{ color: "#8b949e", textAlign: "center", fontSize: "12px" }}>
                        ...
                        >
                            <Typography variant="h6" sx={{ color: "#fff", mb: 2, fontSize: "14px", fontWeight: 600, display: 'flex', alignItems: 'center', gap: 1 }}>
                                <SpeedIcon sx={{ fontSize: 18 }} /> Activity & Feasibility
                            </Typography>
                            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                                <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                                    <Typography variant="body2" sx={{ color: '#8b949e', fontSize: "13px" }}>Last Push</Typography>
                                    <Typography variant="body2" sx={{ color: '#c9d1d9', fontSize: "13px", fontFamily: '"JetBrains Mono", monospace' }}>
                                        {new Date(repoData.pushed_at).toLocaleDateString()}
                                    </Typography>
                                </Box>
                                <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                                    <Typography variant="body2" sx={{ color: '#8b949e', fontSize: "13px" }}>Created</Typography>
                                    <Typography variant="body2" sx={{ color: '#c9d1d9', fontSize: "13px", fontFamily: '"JetBrains Mono", monospace' }}>
                                        {new Date(repoData.created_at).toLocaleDateString()}
                                    </Typography>
                                </Box>
                                <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                                    <Typography variant="body2" sx={{ color: '#8b949e', fontSize: "13px" }}>Status</Typography>
                                    <Chip
                                        ...
                                        sx={{
                                            height: 20,
                                            fontSize: "10px",
                                            bgcolor: repoData.archived ? "error.dark" : "success.dark",
                                            color: "#fff"
                                        }}
                                    />
                                </Box>
                            ...
                            <Typography variant="body2" sx={{ color: '#8b949e', fontSize: "13px", lineHeight: 1.6 }}>
                                To start contributing, fork the repository, clone it locally, and check the <b>CONTRIBUTING.md</b> file for setup instructions.
                            </Typography>
                        </Card>
                    </Box>
                </Grid>
                ...
                <Grid item xs={12} md={8}>
                    <Box sx={{ display: "flex", flexDirection: "column", gap: 2, height: "100%" }}>
                        ...
                        >
                            <Box sx={{ mb: 3 }}>
                                <Typography variant="h6" sx={{ color: "#fff", fontSize: "14px", fontWeight: 600, display: 'flex', alignItems: 'center', gap: 1 }}>
                            ...
                            <Grid container spacing={2}>
                                ...
                                <Grid item xs={6} md={3}>
                                    <Box sx={{
                                        ...
                                        <Typography variant="h4" sx={{ color: '#fff', fontFamily: '"JetBrains Mono", monospace', fontSize: "24px", mb: 0.5 }}>
                                            {openIssuesCount !== null ? openIssuesCount : (repoData.open_issues_count || "-")}
                                        </Typography>
                                        <Typography variant="caption" sx={{ color: '#8b949e', fontSize: "12px" }}>Open Issues</Typography>
                                    </Box>
                                </Grid>
                                ...
                                <Grid item xs={6} md={3}>
                                    <Box sx={{
                                        ...
                                        <Typography variant="caption" sx={{ color: '#8b949e', fontSize: "12px" }}>Forks</Typography>
                                    </Box>
                                </Grid>
                                    ...
                                    >
                                        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 1 }}>
                                            <Box>
                                                ...
                                                <Typography sx={{ color: '#8b949e', fontSize: "12px", fontWeight: 600, whiteSpace: "nowrap" }}>Good First Issues</Typography>
                                            </Box>
                                            <LaunchIcon sx={{ fontSize: 16, color: '#8b949e', mt: 1 }} />
                                        </Box>
                                        <Typography variant="caption" sx={{ color: '#6e7681', fontSize: "11px" }}>Perfect for beginners</Typography>
                                    </Box>
                                </Grid>
                                    ...
                                    >
                                        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 1 }}>
                                            <Box>
                                                ...
                                                <Typography sx={{ color: '#8b949e', fontSize: "12px", fontWeight: 600, whiteSpace: "nowrap" }}>Help Wanted</Typography>
                                            </Box>
                                            <LaunchIcon sx={{ fontSize: 16, color: '#8b949e', mt: 1 }} />
                                        </Box>
                                        <Typography variant="caption" sx={{ color: '#6e7681', fontSize: "11px" }}>General contributions</Typography>
                                    </Box>
                                </Grid>
                            </Grid>
                        </Card>
                        ...
                        >
                            <Box sx={{ p: 2, borderBottom: "1px solid rgba(255, 255, 255, 0.1)", backgroundColor: "rgba(255,255,255,0.03)" }}>
                                <Typography variant="subtitle1" sx={{ color: "#fff", fontWeight: 600, fontSize: "14px", display: 'flex', alignItems: 'center', gap: 1 }}>
                                    ...
                                    {checks.map((check) => (
                                        <Grid item xs={12} sm={6} key={check.name}>
                                            ...
                                            >
                                                <Box sx={{ mt: 0.5 }}>
                                                    {check.passed ? (
                                                        <CheckCircleIcon sx={{ color: "#238636", fontSize: 22 }} />
                                                    ) : (
                                                        <CancelIcon sx={{ color: "#da3633", fontSize: 22 }} />
                                                    )}
                                                </Box>
                                                <Box>
                                                    <Typography sx={{ color: "#c9d1d9", fontWeight: 600, fontSize: "14px", mb: 0.5 }}>
                                                        {check.name}
                                                    </Typography>
                                                    <Typography sx={{ color: "#8b949e", fontSize: "12px", lineHeight: 1.5 }}>
                                                        {check.description}
                                                    </Typography>
                                                </Box>
                                            </Box>
