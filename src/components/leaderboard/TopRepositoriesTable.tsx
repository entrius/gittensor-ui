...
import { getRepositoryChartOption } from './chartFormatters';
import {
  bodyCellStyle,
  getRankIcon,
  headerCellStyle,
...
const TopRepositoriesTable: React.FC<TopRepositoriesTableProps> = ({
  ...
  const {
    ...
    useLogScale,
    setUseLogScale,
    handleChangePage: baseHandleChangePage,
    handleChangeRowsPerPage: baseHandleChangeRowsPerPage,
  } = useTableControls({
    initialSearchQuery: urlSearch,
    initialPage: urlPage >= 0 ? urlPage : 0,
    initialRowsPerPage: VALID_ROWS.includes(urlRows) ? urlRows : 10,
  });
  ...
  const chartOption = useMemo(
    () =>
      getRepositoryChartOption({
        repositories: filteredRepositories,
        useLogScale,
      }),
    [filteredRepositories, useLogScale],
  );

  const handleChangePage = (_event: unknown, newPage: number) => {
    baseHandleChangePage(_event, newPage);
  ...
  const handleChangeRowsPerPage = (
    ...
    baseHandleChangeRowsPerPage(event);
