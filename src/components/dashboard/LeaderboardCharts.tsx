...
import { Box, Card, Tabs, Tab, FormControlLabel, Switch, Typography, Stack, Button } from "@mui/material";
...
const LeaderboardCharts: React.FC = () => {
    ...
    const TierFilterButton = ({ label, value, color }: { label: string; value: typeof tierFilter; color: string }) => (
        <Button
            size="small"
    ...
    const getPRsChartOption = () => {
        const filteredData = tierFilter === "all" ? topPRs : topPRs.filter((pr) => pr.tier === tierFilter);
        const chartData = filteredData.filter(item => parseFloat(item?.score || "0") >= 1).slice(0, 50);
        ...
        return {
          ...
          `;
            ...
            grid: { left: "3%", right: "3%", bottom: "2%", top: "18%", containLabel: true },
            xAxis: {
                type: "category",
                data: xAxisData,
                axisLabel: {
                ...
                axisLine: { lineStyle: { color: "rgba(255, 255, 255, 0.08)", width: 1 } },
                axisTick: { show: false },
            },
            yAxis: {
                ...
                splitLine: { lineStyle: { color: "rgba(255, 255, 255, 0.08)", type: "dashed", opacity: 0.5 } },
                axisLine: { show: false },
                axisTick: { show: false },
            },
            series: [
                {
                    name: "Stems",
                    type: "bar",
                    data: dotData.map((item) => ({ ...item, itemStyle: { color: getTierColor(item.tier), opacity: 0.4, borderRadius: [2, 2, 0, 0] } })),
                    barWidth: 2,
                    z: 1,
    ...
    const getReposChartOption = () => {
        const filteredData = tierFilter === "all" ? repoStats : repoStats.filter((repo) => repo.tier === tierFilter);
        const chartData = filteredData.filter(item => item.totalScore >= 1).slice(0, 50);

        const xAxisData = chartData.map((item) => truncateText(item.repository.split("/")[1] || item.repository, 12));
        const dotData = chartData.map((item) => ({
          ...
          `;
            ...
            yAxis: {
                ...
                axisLabel: {
                    ...
                    fontFamily: "JetBrains Mono",
                    fontSize: 11,
                    formatter: (value: number) => (value < 0.01 ? value.toExponential(1) : value.toFixed(0)),
                },
                splitLine: { lineStyle: { color: "rgba(255, 255, 255, 0.08)", type: "dashed", opacity: 0.5 } },
                axisLine: { show: false },
                axisTick: { show: false },
    ...
    return (
        <Card
            ...
            elevation={0}
        >
            <Box sx={{ borderBottom: "1px solid rgba(255, 255, 255, 0.1)", display: "flex", justifyContent: "space-between", alignItems: "center", px: 2 }}>
                <Tabs
                    value={activeTab}
                ...
                <Box sx={{ display: "flex", gap: 2, alignItems: "center" }}>
                    ...
                    <FormControlLabel
                        ...
                        label={
                            <Typography variant="body2" sx={{ fontFamily: "JetBrains Mono", fontSize: "0.8rem", color: "rgba(255, 255, 255, 0.7)" }}>
                                Log Scale
                            </Typography>
                        }
                    />
            ...
            <Box sx={{ flex: 1, p: 2, backgroundColor: "rgba(0,0,0,0.2)" }}>
                {activeTab === 0 && <ReactECharts option={getPRsChartOption()} style={{ height: "100%", width: "100%" }} />}
                {activeTab === 1 && <ReactECharts option={getReposChartOption()} style={{ height: "100%", width: "100%" }} />}
            </Box>
        </Card>
    );
};
