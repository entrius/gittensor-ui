...
import { Box, Card, Typography, Grid, CircularProgress, Chip, Tooltip } from "@mui/material";
import { ActivityCalendar } from "react-activity-calendar";
import ReactECharts from "echarts-for-react";
import { useAllMinerStats, useAllMinerData, useReposAndWeights } from "../../api";
...
const GlobalActivityViz: React.FC = () => {
    ...
    const { contributionData, contributionsLast30Days, totalDaysShown } = useMemo(() => {
        if (!allPrs || allPrs.length === 0) return { contributionData: [], contributionsLast30Days: 0, totalDaysShown: 0 };
    ...
    const aggregatedStats = useMemo(() => {
        if (!allMinerStats) return { merged: 0, open: 0, closed: 0, globalCredibility: 0 };
    ...
    if (isLoadingPRs || isLoadingStats) {
        return (
            <Card sx={{ borderRadius: 3, border: "1px solid rgba(255, 255, 255, 0.1)", backgroundColor: "transparent", p: 4, display: "flex", justifyContent: "center" }}>
                ...
                <Grid item xs={12} md={3} sx={{
                    ...
                    <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1.5, justifyContent: 'center' }}>
                        {topRepos.slice(0, 11).map((repo) => (
                            <Tooltip key={repo.fullName} title={`${repo.fullName} (Score: ${repo.totalScore.toFixed(0)})`} arrow>
                                <Box
                                    onClick={() => navigate(`/miners/repository?name=${encodeURIComponent(repo.fullName)}`)}
                                    sx={{
                                        textDecoration: "none",
                                        cursor: 'pointer'
                                    }}
